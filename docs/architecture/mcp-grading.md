...
任意支持 STDIO MCP 的编程助手通过本机 STDIO MCP 调用仅监听 loopback 的 FastAPI `/api/mcp`，教师的正常路径固定为：
...
保存请求可携带 `client` 字段标识评分来源，由启动脚本通过 MCP 配置中的可选参数注入，后端记录在 `mcp_metadata.client` 供网页审计展示；未提供时按「外部编程助手」显示。

## 独立复核（大模型复核评分建议）

建议保存后（`ready_for_review`），教师可要求另一个编程助手任务（或同一客户端的新会话）独立复核该建议：

1. 复核者调用 `open_ai_marking_assignment(submission_id)` 完整读取评分包；`ready_for_review` 状态的评分包 header 携带 `current_assessment`（被复核的建议原文），完整读取后签发与评分共用的 `grading_handle`。
2. 复核者按同一 `grading_policy` 独立核对每个评分项（不预设建议正确），随后调用 `save_ai_marking_assessment_review(submission_id, grading_handle, verdict, summary, items, confidence)`：
   - `verdict` 为总体结论：`agree`（全部同意）/ `partial`（部分分歧）/ `disagree`（整体不可靠）。
   - `items` 逐项引用当前 rubric 的 `rubric_item_id`，携带 `verdict`（agree/disagree）与 `comment`；分歧项可附不超过该项满分的 `suggested_score`。
3. 服务端确定性校验：句柄有效且 `context_complete`、状态为 `ready_for_review`（已定稿返回 409）、句柄绑定的 revision/context hash 仍匹配（建议更新后返回 409，须重新打开）、`items` 精确覆盖当前 rubric 全部 item。通过后写入 `submissions.assessment_review`（含 `reviewed_revision` 与服务端补全的 criterion/max_score），并发送状态事件让前端刷新详情。
4. 复核不修改建议本身、不改变状态、不参与最终成绩计算；教师在网页参考复核结论确认最终成绩。建议更新（revision 递增）后旧复核保留但界面标记为过期。

## 运行与诊断

在客户端 MCP 配置中填写 `scripts/run-ai-marking-mcp` 的绝对路径；可选第一个参数作为小写客户端标识。然后运行 `./start.sh`。
题目 rubric 由编程助手从 OCR 提取、服务端确定性校验后写入权威快照，MCP 保存评分时只能引用当前快照，不能上传自由文本 rubric。FastAPI 与 MCP 服务升级 API 版本后必须同时重启，不提供旧版本兼容 shim。
